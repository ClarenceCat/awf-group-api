...
router.get("/", requireAuth, async (req, res) => {
  ...
  try {
    ...
    return res
      .status(500)
      .send({ error: "Error retrieving the Projects for the user" });
  }
});
...
router.post("/", requireAuth, async (req, res) => {
  ...
  if (!title || !description) {
    return res
      .status(400)
      .send({ error: "Missing credentials to create a project" });
  }
  ...
  try {
    ...
    return res
      .status(500)
      .send({ error: "There was an error attempting to create a new project" });
  }
});
  ...
  try {
    ...
    if (!project) {
      return res.status(401).send({
        error: `You are not a member of any projects with the id ${id}`,
      });
    }
    ...
    for (let member_id of project.members) {
      ...
      if (member_found) {
        ...
        members.push({
          firstName: member_found.firstName,
          lastName: member_found.lastName,
          email: member_found.email,
        });
      }
    }
    ...
    for (let task of project.tasks) {
      ...
      const add_task = {
        ...
        created: new Date(task.created).toISOString().slice(0, 10),
      };

      // check if the task has a due date
      if (task.dueDate) {
        add_task["due_date"] = new Date(task.dueDate)
          .toISOString()
          .slice(0, 10);
      } else {
        add_task["due_date"] = "";
...
router.put("/:id", requireAuth, async (req, res) => {
  ...
  try {
    ...
    if (!updated_project) {
      return res
        .status(400)
        .send({ error: "Could not find Project to update" });
    }
...
router.delete("/:id", requireAuth, async (req, res) => {
  ...
  try {
    ...
    if (!deleted) {
      return res
        .status(400)
        .send({ error: "Could not find the project to delete" });
    }
...
router.post("/:id/tasks", requireAuth, async (req, res) => {
  ...
  try {
    ...
    const insert_into_project = await Project.findOneAndUpdate(
      { _id: id, members: user._id },
      { $push: { tasks: new_task } },
      { new: true }
    );
    ...
    const inserted_task =
      insert_into_project.tasks[insert_into_project.tasks.length - 1];
    ...
    if (inserted_task.dueDate) {
      ret_task["due_date"] = new Date(inserted_task.dueDate)
        .toISOString()
        .slice(0, 10);
    } else {
      ret_task["due_date"] = "";
...
router.put("/:project_id/tasks/:task_id", requireAuth, async (req, res) => {
  ...
  if (!title && !description && !due_date) {
    return res
      .status(400)
      .send({ error: "Must specify at least one value to update" });
  }
  ...
  try {
    ...
    const update = await Project.findOneAndUpdate(
      { _id: project_id, members: user._id, "tasks._id": task_id },
      { $set: update_fields },
      { new: true }
    );
    ...
    if (!update) {
      return res
        .status(400)
        .send({ error: "could not update the specified task" });
    }
    ...
    if (updated_task.dueDate) {
      ret_task["due_date"] = new Date(updated_task.dueDate)
        .toISOString()
        .slice(0, 10);
    } else {
      ret_task["due_date"] = "";
...
router.post(
  "/:project_id/tasks/:task_id/assigned",
  requireAuth,
  async (req, res) => {
    // retrieve user from req
    const { user } = req;
    ...
    try {
      ...
      if (!assigned_user) {
        return res
          .status(400)
          .send({ error: `User with email ${email} does not exist` });
      }
      ...
      const updated_project = await Project.findOneAndUpdate(
        { _id: project_id, members: assigned_user._id, "tasks._id": task_id },
        { $addToSet: { "tasks.$.assignedTo": assigned_user._id } },
        { new: true }
      );
      ...
      if (!updated_project) {
        return res.status(400).send({
          error:
            "User is not a member of the project or is already assigned to the Task",
        });
      }
      ...
      return res
        .status(500)
        .send({ error: "There was an error assigning a user to this task" });
    }
  }
);
...
router.delete(
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (!alreadyExists) {
        return res.status(400).send({
          error: `${assigned_user.firstName} ${assigned_user.lastName} is not assigned to this task`,
        });
      }
      ...
      const updated_project = await Project.findOneAndUpdate(
        { _id: project_id, members: assigned_user._id, "tasks._id": task_id },
        { $pull: { "tasks.$.assignedTo": assigned_user._id } },
        { new: true }
      );
...
router.post("/:id/members", requireAuth, async (req, res) => {
  ...
  try {
    ...
    if (!new_member) {
      return res.status(400).send({
        error: "A user with the email you have specified does not exist",
      });
    }

    // check if the user is alread a member
    const check_member = await Project.findOne({
      _id: id,
      members: new_member._id,
    });

    // check if the user is already a member
    if (check_member) {
      return res.status(400).send({
        error: `A person with the email ${email} is already a member of this project`,
      });
    }
    ...
    if (!update) {
      return res
        .status(400)
        .send({ error: `Failed to add user with email ${email} to the group` });
    }
    ...
    return res
      .status(500)
      .send({ error: "Could not add a new member to the project" });
  }
});
...
router.delete("/:id/members", requireAuth, async (req, res) => {
  ...
  try {
    ...
    if (!find_proj) {
      return res
        .status(401)
        .send({ error: "You do not have access to this project" });
    }

    // remove the member from the project
    const updated = await Project.findOneAndUpdate(
      { _id: id, members: rem_user._id },
      { $pull: { members: rem_user._id } },
      { new: true }
    );

    if (!updated) {
      return res.status(400).send({
        error: `No user with the email ${email} can be removed from the project`,
      });
    }
